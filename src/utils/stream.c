...
static int my_write(int fd, void *buf, unsigned n) {
    ...
    if(p2p_addr == 0) {
		...
		*/
		...
		return crb_tcp_send_frame(fd, buf, n);
    }
...
int IMP_Encoder_GetStream(int encChn, IMPEncoderStream *stream, int blockFlag) {
	...
	if(func == 0) {
		init++;
		void *next = dlsym(RTLD_NEXT, "IMP_Encoder_GetStream");
		put("next", next);
		void *sym = dlopen(LIB, RTLD_GLOBAL|RTLD_LAZY);
		put("sym2", sym);
		//func = dlsym(RTLD_NEXT, "IMP_Encoder_GetStream");
		func = dlsym(sym, "IMP_Encoder_GetStream");
		put("func2", func);
	}
	if(func) {
		rval = (func)(encChn, stream, blockFlag);
		crb_tcp_hook(rval, encChn, (void *)stream, blockFlag);
	}
	return rval;
}
*/
...
public int IMP_Encoder_GetStream(int encChn, IMPEncoderStream *stream, int blockFlag) {
	...
	if(func) {
		...
		if(p2p) {
			crb_p2p_hook(rval, encChn, (void *)stream, blockFlag);
		} else {
			crb_tcp_hook(rval, encChn, (void *)stream, blockFlag);
		}
  		return rval;
