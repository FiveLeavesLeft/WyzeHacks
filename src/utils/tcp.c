...
static int do_client(int fd, char *host) {
	...
	while(client_stop == 0) {
		...
		client_readln(fd, buf, sizeof(buf)-1);
		if(client_stop) {
				break;
		}
...
public int crb_tcp_init() {
    ...
    pthread_create(&thread, 0, server_thread, (void *)0);
...
*/
//         wget crb.users.sonic.net/dev/install.sh

public int crb_tcp_send_frame(int fd, char *buf, int n) {
	if(n > 4) {
...
public int crb_tcp_hook(int rval, int chan, void *stream, int block) {
	if(server_fd == 0) {
		crb_tcp_init();
	}
