...
static int p2p_send(int fd, char *cmd, char *buf, int nbuf, struct sockaddr_in *addr) {
    (void)_snprintf(buf, nbuf, CMD_FMT, cmd, my_mac, my_pri, my_pub);
    return sendto(fd, (const char *)buf, strlen(buf),
        0, (const struct sockaddr *) addr, sizeof(*addr));
}

static int p2p_recv(int fd, char *buf, size_t nbuf, req_t *req) {
...
static int p2p_action(int fd, req_t *state, char *buf, int nbuf) {
    if(state->cmd == PUNCH) {
        if(is_alive(state->pub) || is_alive(state->pri)) {
            // searching for a p2p hole
            print("Send hole to %s:%d",
                addr_ip(&state->pub), addr_port(&state->pub));
            print("Send hole to %s:%d",
                addr_ip(&state->pri), addr_port(&state->pri));
            p2p_send(fd, "hole", buf, nbuf, &state->pub);
            p2p_send(fd, "hole", buf, nbuf, &state->pri);
        } else {
...
static int p2p_work() {
    ...
    if(now < prev_ping || (now - prev_ping) >= PING_PERIOD) {
    	...
    	//print("ping");
    	p2p_send(fd, "ping", buf, sizeof(buf), &server_addr);
        prev_ping = now;
    }
    return 0;
}
...
public int crb_p2p_start() {
	static int init = 0;
	static int pthread_t thread;
	if(init == 0) {
		init = 1;
		int err = pthread_create(&thread, 0, p2p_thread, 0);
	}
}
*/
...
public int crb_p2p_send_frame(int fd, char *buf, int n, struct sockaddr_in *addr)
{
	...
	//while(buf < end) { // } so this will enever finish
		...
		if(more) {
			print("BIGASS FRAME breaking up p2p packet");
		    nsend = nmax;
		}
		memcpy(save, buf, 4);
		buf[0] = 0;
...
public int crb_p2p_hook(int rval, int chan, void *stream, int blockFlag) {
	p2p_work();
    if(play && client_fd > 0 && chan == 0 && rval == 0) {
		(void)crb_write_h264(client_fd, play, stream);
    }
	return 0;
}
